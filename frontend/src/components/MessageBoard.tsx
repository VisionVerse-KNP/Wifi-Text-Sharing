    ...
    >
        ...
        <div className="flex flex-col gap-1 rounded-xl bg-brand-500/5 border border-brand-500/20 px-3 py-2">
          {pinnedMessages.map((m) => (
            <button
              ...
              className="min-w-0 text-xs text-left hover:underline flex items-center gap-1.5 w-full"
            >
              <span className="shrink-0">📌</span>
              <span className="font-medium text-brand-600 dark:text-brand-300 shrink-0">{m.userName}:</span>
              <span className="text-slate-500 dark:text-slate-400 truncate min-w-0 flex-1">{m.text}</span>
            </button>
          ))}
        </div>
      )}

      <div className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-thin flex flex-col gap-3 pr-1">
        {filtered.length === 0 && (
      ...
      <div className="relative flex items-center gap-2">
        ...
        <button
          ...
          title="Attach a file"
          className="w-10 h-10 shrink-0 rounded-full bg-white/70 dark:bg-slate-800/60 border
            border-slate-200 dark:border-slate-700 flex items-center justify-center hover:bg-white
            dark:hover:bg-slate-800 transition text-slate-600 dark:text-slate-300"
        ...
        <div className="relative flex-1 min-w-0">
          <textarea
            ...
            rows={1}
            className="w-full min-h-[42px] resize-none rounded-2xl pl-4 pr-14 py-2.5 bg-white/70 dark:bg-slate-800/60 border
              border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500 text-sm
              overflow-y-auto scrollbar-thin max-h-32 leading-6 box-border"
          />
          <span className="pointer-events-none absolute bottom-1.5 right-3.5 text-[10px] text-slate-400">
